...
if (localStorage.getItem('mbbs_submitted') === 'true') {
  ...
  // University rendering (your full DB - unchanged, but extended with descriptions)
...
document.getElementById('dynamic-country').textContent = country;
    // Complete university database based on provided list - EXTENDED WITH DESCRIPTIONS
    const db = {
      "Georgia": [
    ...
    const arr = db[country] || db["Georgia"];
    arr.forEach((u)=>{
  ...
  function renderUniversityDetails() {
    ...
    // Reuse db from above - find the uni
    const db = {
      // ... (same db as in renderUniversities - omitted for brevity, but include full in actual code)
    }; // Paste the full db here again or make it global

    const uniData = db[country]?.find(u => u.name === uniName);
    if (!uniData) {
