...
// ==== PUT THIS AT THE VERY TOP of script.js (replace the old reload block) ====
if (localStorage.getItem('mbbs_submitted') === 'true') {
  ...
  let userName = 'there';
  let userCountry = 'abroad';

  if (saved) {
    try {
      const data = JSON.parse(saved);
      if (data.name && data.name.trim()) userName = data.name.trim();
  ...
  // Optional: hide Restart button completely after submission
...
if (j.success) {
    ...
    const userCountry = state.country || 'abroad';
